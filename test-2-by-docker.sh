...
if [ ! -d ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/nginx/sbin ]; then	
	...
	if [ ! -d ${OPENSHIFT_HOMEDIR}/app-root/runtime/www ]; then
      mkdir ${OPENSHIFT_HOMEDIR}/app-root/runtime/www
    fi
...
http {
    ...
    server {
		...
		root   $OPENSHIFT_HOMEDIR/app-root/runtime;
		...
		location /www {
		#location ~* ^/centova(.*) {
            #root   $OPENSHIFT_HOMEDIR/app-root/runtime/www;
			alias   $OPENSHIFT_HOMEDIR/app-root/runtime/www;
...
if [ ! -d ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/centovacast ]; then	
	cd $OPENSHIFT_TMP_DIR && rm -rf centovacast*
    wget http://rhuangonzaga.com/files/centova/centovacast-2.2.6-lin64.tar.gz

	tar xzvf centovacast-*
	...
	cp -R www/. ${OPENSHIFT_HOMEDIR}/app-root/runtime/www/
	
	if [ ! -d ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/centovacast ]; then
      mkdir ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/centovacast
    fi
	cp -R  ${OPENSHIFT_TMP_DIR}/centovacast*/. ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/centovacast
	#echo "export path"
	#export PATH=${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/sbin:$PATH
	cd ${OPENSHIFT_TMP_DIR}/centovacast* && cp ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/ioncube/loader-wizard.php ./loader-wizard.php
	...
	echo "[xdebug]
	zend_extension = $OPENSHIFT_HOMEDIR/app-root/runtime/srv/ioncube/ioncube_loader_lin_5.4.so
	" >> "${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/lib/php.ini"
	#sed -i "s@$OPENSHIFT_HOMEDIR@"${OPENSHIFT_HOMEDIR}"@g"  ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/lib/php.ini	
	kill -9 `lsof -t -i :9000`
	${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/sbin/php-fpm 
	 '
...
#	make && make install
	...
	if [ ! -d ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/ioncube ]; then	
	   cd $OPENSHIFT_TMP_DIR
       wget http://downloads2.ioncube.com/loader_downloads/ioncube_loaders_lin_x86-64.tar.gz
	   tar xzvf ioncube_loaders_lin_x86-64.tar.gz && rm -rf ioncube_loaders_lin_x86-64.tar.gz
	   
	  if [ ! -d ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/ioncube ]; then
         mkdir ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/ioncube
      fi
	   mv  ioncube*/* ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/ioncube
	  echo "export path"
	  export PATH=${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/ioncube:$PATH
    fi
	...
	cp ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/etc/php-fpm.conf.default	${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/etc/php-fpm.conf
	sed -i "s@127.0.0.1:9000@"${OPENSHIFT_DIY_IP}:9000"@g"  ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/etc/php-fpm.conf	
    sed -i "s@user = nobody@;user = nobody@g"  ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/etc/php-fpm.conf	
	sed -i "s@group = nobody@;group = nobody@g"  ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/etc/php-fpm.conf	
	#kill -9 `lsof -t -i :8080`
	#nohup sh -c "${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/sbin/php-fpm --fpm-config ${OPENSHIFT_HOMEDIR}/app-root/runtime/srv/php-${PHP_VERSION}/etc/php-fpm.conf"  > $OPENSHIFT_LOG_DIR/php_run.log 2>&1 
...
sudo ./configure  --prefix=$OPENSHIFT_REPO_DIR/srv/icegenerator --bindir=$OPENSHIFT_REPO_DIR/srv/icegenerator/bin #--with-shout=$OPENSHIFT_REPO_DIR/srv/libshout
make 
make install
...
cd lame-3.99.5
sudo ./configure  --prefix=$OPENSHIFT_REPO_DIR/srv/lame
make
make install 
...
cd sox-14.4.2
sudo ./configure  --prefix=$OPENSHIFT_REPO_DIR/srv/SoX #--with-lame=$OPENSHIFT_REPO_DIR/srv/lame
make -s
make install 
...
wget http://revueperiode.net/mp3/CCMaherP%c3%a9riode.mp3
...
sudo ./configure --prefix=$OPENSHIFT_REPO_DIR/srv/libtool 
 make
make install
...
#autoreconf --install
sudo ./configure --prefix=$OPENSHIFT_REPO_DIR/srv/libmnl 
 make
make install
...
sudo ./configure --prefix=$OPENSHIFT_REPO_DIR/srv/bison 
make
make install
...
sudo ./configure --prefix=$OPENSHIFT_REPO_DIR/srv/flex 
make
make install
...
sudo ./configure --prefix=$OPENSHIFT_REPO_DIR/srv/autoconf 
make
make install
...
sudo ./configure --prefix=$OPENSHIFT_REPO_DIR/srv/automake 
make
make install
...
sudo ./configure --prefix=$OPENSHIFT_REPO_DIR/srv/libnfnetlink 
make
make install
...
aclocal && autoheader && automake && autoconf && autoreconf -i
autoreconf --install
sudo ./configure --prefix=$OPENSHIFT_REPO_DIR/srv/libnetfilter_conntrack \
...
sudo ./configure --prefix=$OPENSHIFT_REPO_DIR/srv/iptables --enable-devel --enable-libipq
make
make install
